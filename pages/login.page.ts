...
export class RegistrationPage {
  ...
  constructor(private page: Page) {
    ...
    this.conpassword = this.page.getByTestId('password-input-Password Confirmation');
    ...
    this.errfirstname = this.page.getByText('Please enter first name', { exact: true });
    this.errlastname = this.page.getByText('Please enter last name', { exact: true });
    this.errusername = this.page.getByText('Please enter username', { exact: true });
    this.errusername1 = this.page.getByText('Username must be at least 2 characters', { exact: true });
    this.errusername2 = this.page.getByText('Username is already taken', { exact: true });

    this.erremail = this.page.getByText('Please enter email address', { exact: true });
    this.erremail1 = this.page.getByText('Please enter valid email address', { exact: true });

    this.errpassword = this.page.getByText('Password is required', { exact: true });
    this.errpassword1 = this.page.getByText('Password must have a minimum of 12 characters', { exact: true });
    this.errpassword2 = this.page.getByText('Password must contain at least: 1 lower-case character', { exact: true });
    this.errpassword3 = this.page.getByText('Password must contain at least: 1 upper-case character', { exact: true });
    this.errpassword4 = this.page.getByText('Password must contain at least: 1 digit', { exact: true });
    this.errpassword5 = this.page.getByText('Password must contain at least: 1 symbol', { exact: true });

    this.errconpassword1 = this.page.getByText('Passwords do not match', { exact: true });
    this.errconpassword = this.page.getByText('Please re-enter your password', { exact: true });
  ...
  async validation(timeout = 120000) {
    ...
    await expect(this.page.getByText('Username must be at least 2 characters', { exact: true })).toBeVisible();

    await this.username.fill('Admin');
    await helper.checkColor(this.username, this.greyColour, "border");
    await this.email.click();
    await helper.checkColor(this.errusername2, this.redColour, "color");
    await expect(this.page.getByText('Username is already taken', { exact: true })).toBeVisible();

    await this.username.clear();
    await this.username.fill('Admin123'); // Now valid username

    await this.email.fill('joseph');
    await expect(this.page.getByText('Please enter valid email address', { exact: true })).toBeVisible();
    ...
    await expect(this.page.getByText('Password must have a minimum of 12 characters', { exact: true })).toBeVisible();
    ...
    await expect(this.page.getByText('Password must contain at least: 1 digit', { exact: true })).toBeVisible();
    await helper.checkColor(this.password, this.greyColour, "border");
    await this.conpassword.click();
    await helper.checkColor(this.errpassword4, this.redColour, "color");

    await this.password.fill('1234567890123');
    await expect(this.page.getByText('Password must contain at least: 1 lower-case character', { exact: true })).toBeVisible();
    await helper.checkColor(this.password, this.greyColour, "border");
    await this.conpassword.click();
    await helper.checkColor(this.errpassword2, this.redColour, "color");

    await this.password.fill('1234567890123a');
    await expect(this.page.getByText('Password must contain at least: 1 upper-case character', { exact: true })).toBeVisible();
    await helper.checkColor(this.password, this.greyColour, "border");
    await this.conpassword.click();
    await helper.checkColor(this.errpassword3, this.redColour, "color");

    await this.password.fill('1234567890123aB');
    await expect(this.page.getByText('Password must contain at least: 1 symbol', { exact: true })).toBeVisible();
    await helper.checkColor(this.password, this.greyColour, "border");
    await this.conpassword.click();
